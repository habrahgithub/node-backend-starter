...
- 2026-02-20: Enforced local-first connector policy by removing cloud-connector references from active control-plane/specs, removing connector checkup execution from root `package.json`, deleting connector-specific repo/skill assets from this workspace, and introducing a single deferred connector marker in `docs/ROADMAP_CONNECTORS.md`.
...
- 2026-03-11: Standardized repo cleanup on a non-destructive source-of-truth-first flow: audit all visible git roots, reconcile duplicate remotes by fast-forward plus parent gitlink sync, capture evidence under `artifacts/repo-cleanup/`, and defer archive/delete actions until explicitly approved and unrelated dirty trees are resolved.
